# Orchestrator & Handoff Reference — Home Services ADK Agent

## Full Pipeline with Conditional Routing

Google ADK supports two patterns for conditional handoffs:
1. **`SequentialAgent`** — linear, no branching (simple)
2. **`LlmAgent` as router** — uses LLM to decide next agent (flexible)

For Intent → Clarification conditional, use pattern 2.

---

## Pattern: LlmAgent Router + Sub-Agents
...
from agents.negotiation_agent import create_negotiation_agent
...
|-----------|-----------|---------|
| IntentUnderstandingAgent | confidence ≥ 0.6 AND location not null | ProviderDiscoveryAgent |
| IntentUnderstandingAgent | confidence < 0.6 OR location null OR service = "Other" | ClarificationAgent |
| ClarificationAgent | After collecting missing info | IntentUnderstandingAgent (re-run) |
| ProviderDiscoveryAgent | Always (if providers found) | RankingAgent |
| ProviderDiscoveryAgent | No providers found | Orchestrator (notify user, stop) |
| RankingAgent | Always | NegotiationAgent |
| NegotiationAgent | confirmed | BookingAgent |
| NegotiationAgent | no_provider_available | Orchestrator (notify user, stop) |
| BookingAgent | Always | FollowUpAgent |
| FollowUpAgent | rating ≥ 4 | elo_update(win) → end |
| FollowUpAgent | rating ≤ 2 OR no-show | elo_update(loss) → escalate_issue |

---

## Session State Keys

ADK persists these in `InMemorySessionService` across agent calls:

```python
# Keys written by each agent via output_key
"intent"                 # IntentOutput dict
"clarification_response" # str
"candidate_providers"    # List[ProviderRecord]
"ranked_providers"       # List[RankedProvider]
"negotiation_result"     # NegotiationResult dict
"booking_result"         # { booking: BookingRecord, confirmation_message: str }
"followup_result"        # FollowUpState dict
```

Access in agent instructions via: `{intent}`, `{candidate_providers}`, etc.
...
async def test():
    ...
    async for event in runner.run_async(
        ...
        new_message={'role': 'user', 'parts': [{'text': 'Geyser leak kar raha hai'}]}
    ):
        if event.is_final_response():
